...
import { Dumbbell, Plus, Edit3, Trash2, X } from 'lucide-react';
import { Package } from '../../types';
import { savePackageData, deletePackageData, PackageFormState } from './packageHelpers';

export default function GymAdmin() {
  const { language, t } = useLanguage();
  const { gymList, packagesList, triggerToast, loadAllData, actionLoading, setActionLoading, setAdminConfirmModal } = useAdminContext();
  ...
  const handleSavePackage = async (e: React.FormEvent, category: 'salon' | 'gym') => {
    ...
    try {
      ...
      setPackageForm({ id: '', name: '', price: '', sessions: '1', category: 'salon' });
      setIsEditingPackage(false);
      loadAllData();
    } catch (err) {
      console.error('[GymAdmin] savePackageData failed', err);
    ...
    setAdminConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          console.error('[GymAdmin] deletePackageData failed', err);
    ...
    <>
              ...
              <div className="md:col-span-1 bg-white border border-olive-light rounded-xl p-6 shadow-sm">
                <h3 className="font-serif text-lg font-bold text-olive-dark border-b border-gray-100 pb-2 mb-4">
                  {isEditingPackage && packageForm.category === 'gym' ? 'Modify Membership' : 'Register Gym Membership'}
                </h3>

                <form onSubmit={e => handleSavePackage(e, 'gym')} className="flex flex-col gap-4">
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-400 mb-1 leading-none font-sans">
                      Membership Name
                    </label>
                    <input
                      id="gym-membership-name"
                      type="text"
                      placeholder="e.g. 1 Month Elite"
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-gray-400 mb-1 leading-none font-sans">
                      Access Session Allowance
                    </label>
                    <input
                      id="gym-membership-sessions"
                      type="number"
                      placeholder="e.g. 1 for pass, or custom"
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    >
                      {actionLoading ? 'Processing...' : (isEditingPackage && packageForm.category === 'gym' ? 'Save Changes' : 'Register Membership')}
                    </button>
                    {isEditingPackage && (
                      <button
                        type="button"
              ...
              <div className="md:col-span-2 bg-white border border-olive-light rounded-xl p-6 shadow-sm">
                <h3 className="font-serif text-lg font-bold text-olive-dark border-b border-gray-100 pb-2 mb-4">
                  Active Gym Memberships & Passes Catalogue
                </h3>
                    ...
                    >
                      <div>
                        ...
                        <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">
                          Access Cycles: {pkg.sessions} count
                        </span>
                      </div>

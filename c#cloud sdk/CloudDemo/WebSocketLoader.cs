...
namespace CloudDemo
{
    public class WebSocketLoader
    {
        ...
        private static GateValidationResult ValidateGateAccess(string qrPayload, string branch)
        {
            ...
            try
            {
                // Read the backend server URL from configuration, default to localhost for testing
                string backendUrl = System.Configuration.ConfigurationManager.AppSettings["HayatBackendUrl"] ?? "http://localhost:3000";
                string gateApiUrl = backendUrl.TrimEnd('/') + "/api/gate/scan";
                
                var request = (HttpWebRequest)HttpWebRequest.Create(gateApiUrl);
                request.Method = "POST";
                request.ContentType = "application/json";
                request.Timeout = 10000;
                request.ReadWriteTimeout = 10000;
                request.UserAgent = "CloudDemo-GateClient/1.0";

                using (var streamWriter = new StreamWriter(request.GetRequestStream()))
                {
                    string json = "{\"qrPayload\":\"" + qrPayload.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\",\"branch\":\"" + branch.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"}";
                ...
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    ...
                    if (string.IsNullOrWhiteSpace(responseBody))
                    {
                        ...
                        result.Reason = "Empty response from gate service";
                        return result;
                    }
                    ...
                    try
                    {
                        ...
                        int? access = responseJson.Value<int?>("access");
                        result.IsAllowed = (success == true) || (access == 1);
                        result.Message = responseJson.Value<string>("message") ?? (result.IsAllowed ? "Welcome" : "Denied");
                        result.CustomerName = responseJson.Value<string>("customerName") ?? string.Empty;
                        result.CustomerId = responseJson.Value<string>("customerId") ?? string.Empty;
                        result.Reason = responseJson.Value<string>("reason") ?? string.Empty;
                    }
                    catch (Exception jsonException)
                    {
                        Console.WriteLine("Gate response parse error: " + jsonException.Message);
                        result.Message = "Denied";
                        result.Reason = jsonException.Message;
                    }
                }
            }
            catch (WebException webException)
            {
                Console.WriteLine("Gate API error: " + webException.Message);
                try
                {
                    using (var errorResponse = webException.Response as HttpWebResponse)
                    using (var errorStream = errorResponse != null ? errorResponse.GetResponseStream() : null)
                    using (var streamReader = errorStream != null ? new StreamReader(errorStream) : null)
                    {
                        if (streamReader != null)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(responseBody))
                            {
                                ...
                                {
                                    JObject responseJson = JObject.Parse(responseBody);
                                    result.Message = responseJson.Value<string>("message") ?? "Denied";
                                    result.Reason = responseJson.Value<string>("reason") ?? responseJson.Value<string>("error") ?? "Gate service rejected the request";
                                    result.CustomerName = responseJson.Value<string>("customerName") ?? string.Empty;
                                    result.CustomerId = responseJson.Value<string>("customerId") ?? string.Empty;
                                }
                                ...
                                {
                                    result.Message = "Denied";
                                    result.Reason = parseException.Message;
                                }
                            }
                        }
                    }
            ...
            {
                Console.WriteLine("Gate API error: " + exception.Message);
                ...
                if(string.IsNullOrEmpty(cmd)==false) //client active send data
                {
                    ...
                    switch (cmd)
                    {
                        ...
                        case "sendlog":        
                            ...
                            var snLog = jsonMsg.Value<string>("sn");
                            var lastGateValidation = new GateValidationResult
                            {
                                IsAllowed = false,
                                Message = "Denied",
                                Reason = "No scan payload"
                            };
                            foreach (var ss in attRecords)
                            {
                               ...
                               if (enrollid > 0)
                               {
                                   lastGateValidation = ValidateGateAccess(enrollid.ToString(), "Janabiya");
                                   if (lastGateValidation.IsAllowed)
                                   {
                                       Console.WriteLine("Validation Success for: " + enrollid + ". Opening door...");
                                       WebSocketLoader.opendoor(snLog);
                                   }
                                   else
                                   {
                                       Console.WriteLine("Validation Failed for: " + enrollid + " - " + lastGateValidation.Message + " (" + lastGateValidation.Reason + ")");
                                   }
                               }
                            }
                             server= DateTime.Now;  //return the servertime to synchronizes the client device time
                             var sendLogResponse = new JObject();
                             sendLogResponse["ret"] = "sendlog";
                             sendLogResponse["result"] = true;
                             sendLogResponse["cloudtime"] = server.ToString();
                             sendLogResponse["access"] = lastGateValidation.IsAllowed ? 1 : 0;
                             sendLogResponse["message"] = lastGateValidation.Message;
                             if (!string.IsNullOrWhiteSpace(lastGateValidation.CustomerName))
                             {
                                 sendLogResponse["customerName"] = lastGateValidation.CustomerName;
                             }
                             if (!string.IsNullOrWhiteSpace(lastGateValidation.CustomerId))
                             {
                                 sendLogResponse["customerId"] = lastGateValidation.CustomerId;
                             }
                             if (!string.IsNullOrWhiteSpace(lastGateValidation.Reason))
                             {
                                 sendLogResponse["reason"] = lastGateValidation.Reason;
                             }
                             strRespone = sendLogResponse.ToString();
                            session.Send(strRespone);
                            break;
                        case "senduser":
                        ...
                        case "sendqrcode":
                             try {
                                 var qrRecord = jsonMsg.Value<string>("record");
                                 Console.WriteLine("QR Scanned: " + qrRecord);
                                 var validation = ValidateGateAccess(qrRecord, "Janabiya");
                                 if (validation.IsAllowed)
                                 {
                                     Console.WriteLine("Validation Success for: " + qrRecord + ". Opening door...");
                                     WebSocketLoader.opendoor(jsonMsg.Value<string>("sn"));
                                 }
                                 else
                                 {
                                     Console.WriteLine("Validation Failed for: " + qrRecord + " - " + validation.Message + " (" + validation.Reason + ")");
                                 }

                                 var responsePayload = new JObject();
                                 responsePayload["ret"] = "sendqrcode";
                                 responsePayload["result"] = true;
                                 responsePayload["access"] = validation.IsAllowed ? 1 : 0;
                                 responsePayload["message"] = validation.Message;
                                 if (!string.IsNullOrWhiteSpace(validation.CustomerName))
                                 {
                                     responsePayload["customerName"] = validation.CustomerName;
                                 }
                                 if (!string.IsNullOrWhiteSpace(validation.CustomerId))
                                 {
                                     responsePayload["customerId"] = validation.CustomerId;
                                 }
                                 if (!string.IsNullOrWhiteSpace(validation.Reason))
                                 {
                                     responsePayload["reason"] = validation.Reason;
                                 }
                                 strRespone = responsePayload.ToString();
                             } catch(Exception e) {
                                 Console.WriteLine("API Error: " + e.Message);
                                 var errorPayload = new JObject();
                                 errorPayload["ret"] = "sendqrcode";
                                 errorPayload["result"] = true;
                                 errorPayload["access"] = 0;
                                 errorPayload["message"] = "Error";
                                 errorPayload["reason"] = e.Message;
                                 strRespone = errorPayload.ToString();
                             }
                             session.Send(strRespone);
                             break;
                        default:

        ...
        if (!desc.includes("(مع:")) {
          ...
          if (createdAt) {
             ...
             if (partnerName) {
                ...
                const q3 = query(collection(db, "auditLogs"), where("timestamp", "==", createdAt), where("customerId", "==", invoice.primaryCustomerId), where("action", "==", "Purchase"));
                const logsSnap = await getDocs(q3);
                for (const logDoc of logsSnap.docs) {
                   ...
                   if (logDesc && logDesc.includes(desc) && !logDesc.includes("(مع:")) {
                      ...
                      await updateDoc(doc(db, "auditLogs", logDoc.id), { description: newLogDesc });
                   }
                }
